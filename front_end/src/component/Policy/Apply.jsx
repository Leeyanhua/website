...
import { getPolicyArticle, postPolicy } from '../../utils/Policy';
import { putColPo } from '../../utils/Users';
...
class Apply extends React.Component {
  ...
  componentWillMount() {
    getPolicyArticle(this.props.params.id, (result) => {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        const { _id, policy_name } = this.state.content;
        values.policy = { _id, name: policy_name };
        postPolicy(values, () => {
          if (this.context.USER._id) {    // 已登录
            putColPo(this.state.content._id, (result) => {
              if (result.code === 0) {
                message.success('政策申请成功，已经自动收藏该政策');
    ...
    return (
      <div>
        <Card className="apply-title">
          <span style={{ float: 'right' }}>{content.release_date}</span>
          <h2 style={{ color: '#3ac1bd' }}>{content.policy_name}</h2>
        </Card>
        <Card className="applyweb">
          <h2 style={{ color: '#3ac1bd' }}>政策申请</h2>
